package com.documentos.wms_beirario.ui.auditoriaEstoque.viewModels

import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import androidx.lifecycle.ViewModelProvider
import androidx.lifecycle.viewModelScope
import com.documentos.wms_beirario.model.auditoriaEstoque.response.response.ListEstantesAuditoriaEstoqueItem
import com.documentos.wms_beirario.model.auditoriaEstoque.response.response.ListaAuditoriasEstoque
import com.documentos.wms_beirario.repository.auditoriaEstoque.AuditoriaEstoqueRepository
import com.documentos.wms_beirario.utils.extensions.validaErrorDb
import com.documentos.wms_beirario.utils.extensions.validaErrorException
import kotlinx.coroutines.launch

class AuditoriaEstoqueViewModel1(val repository: AuditoriaEstoqueRepository) : ViewModel() {

    private var sucessGetAuditoria = MutableLiveData<ListaAuditoriasEstoque?>()
    val sucessGetAuditoriaShow get() = sucessGetAuditoria

    private var sucessGetAuditoriaEmply = MutableLiveData<String>()
    val sucessGetAuditoriaEmplyShow get() = sucessGetAuditoriaEmply

    private var errorDb = MutableLiveData<String>()
    val errorDbShow get() = errorDb

    private var errorAll = MutableLiveData<String>()
    val errorAllShow get() = errorAll

    private var progress = MutableLiveData<Boolean>()
    val progressShow get() = progress

    private var sucessGetEstantes = MutableLiveData<List<ListEstantesAuditoriaEstoqueItem>?>()
    val sucessGetEstantesShow get() = sucessGetEstantes
    fun getAuditorias(idArmazem: Int, token: String) {
        viewModelScope.launch {
            try {
                progress.postValue(true)
                val result = repository.getListaAuditoriasEstoque1(idArmazem = idArmazem,token = token)
                if (result.isSuccessful) {
                    if (result.body()?.isNotEmpty() == true) {
                        sucessGetAuditoria.postValue(result.body())
                    } else {
                        sucessGetAuditoriaEmply.postValue("Sem item para auditoria")
                    }
                } else {
                    errorDb.postValue(validaErrorDb(result))
                }
            } catch (e: Exception) {
                errorAll.postValue(validaErrorException(e))
            }finally {
                progress.postValue(false)
            }
        }
    }

    fun getEstantes(idArmazem: Int, token: String,idAuditoriaEstoque :String) {
        viewModelScope.launch {
            try {
                progress.postValue(true)
                val result = repository.getListEstantes(idArmazem = idArmazem,token = token,idAuditoriaEstoque = idAuditoriaEstoque)
                if (result.isSuccessful) {
                    if (result.body()?.isNotEmpty() == true) {
                        sucessGetEstantes.postValue(result.body())
                    } else {
                        sucessGetAuditoriaEmply.postValue("Sem estantes para auditoria selecionada.")
                    }
                } else {
                    errorDb.postValue(validaErrorDb(result))
                }
            } catch (e: Exception) {
                errorAll.postValue(validaErrorException(e))
            }finally {
                progress.postValue(false)
            }
        }
    }


    class AuditoriaEstoqueViewModel1Factory constructor(private val repository: AuditoriaEstoqueRepository) :
        ViewModelProvider.Factory {
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            return if (modelClass.isAssignableFrom(AuditoriaEstoqueViewModel1::class.java)) {
                AuditoriaEstoqueViewModel1(this.repository) as T
            } else {
                throw IllegalArgumentException("ViewModel Not Found")
            }
        }
    }
}