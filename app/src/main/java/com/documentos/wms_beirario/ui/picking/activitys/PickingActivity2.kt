...
class PickingActivity2 : AppCompatActivity() {
    ...
    private lateinit var mIntentData: ArrayList<ResponsePickingReturnGroupedItem>
    ...
    private fun initIntentData() {
        try {
            if (intent != null) {
                val data =
                    intent.getSerializableExtra("DATA_PICK") as ArrayList<ResponsePickingReturnGroupedItem>
                mIntentData = data
                Log.e("PICKING 2", "initIntentData --> $mIntentData ")
            }
        } catch (e: Exception) {
